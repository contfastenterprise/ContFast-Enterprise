...
// respondia `success: false`, ni se miraba: mismo resultado, sin siquiera el
// aviso.
//
...
const PANTALLAS: [string, string, string][] = [
  ['src/app/dashboard/ecf/page.tsx', 'e-CF', 'fetchInvoices'],
  ['src/app/dashboard/invoices/page.tsx', 'facturas', 'loadInvoices'],
  ['src/app/dashboard/ap/page.tsx', 'CxP', 'fetchData'],
];
for (const [ruta, nombre, recarga] of PANTALLAS) {
  ...
  ok(
    `${nombre}: el error se pinta en lugar del vacio, con su recarga`,
    tc.includes('errorCarga ? (') && tc.includes(`<ErrorDeCarga mensaje={errorCarga} onReintentar={${recarga}} />`)
  );
}
