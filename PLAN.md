...
- **Lector de Facturas Inteligente (OCR - Uploader Refactored)**: Refactorización completa del componente `InvoiceImageUploader.tsx` (reduciendo la lógica embebida en la UI) mediante la extracción de todo su estado, gestores de eventos de arrastrar y soltar, procesamiento canvas de optimización/compresión de imágenes de entrada, e invocación a la API OCR del backend en el hook desacoplado y reutilizable `useInvoiceOcr.ts`.
...
- **Sincronización de Fechas de Filtro e Inicialización en el Dashboard**: Corregida la inicialización de fecha en `src/app/dashboard/purchases/page.tsx` usando una función local que calcula la fecha actual en la zona horaria del cliente en lugar de utilizar `.toISOString().split('T')[0]`. Adicionalmente, se cambió el filtro de fecha de inicio (`filterStartDate`) para que apunte por defecto al día de hoy (en lugar del día 1 del mes) y se agregó un hook de efecto para realizar automáticamente la búsqueda inicial en el montaje del componente, de modo que al cargar la página se seleccionen de forma inmediata las transacciones del día actual.
...
### 42. Suite Completa del Ciclo Contable Profesional (República Dominicana)
- **Base de Datos y Modelado**: Se ampliaron las tablas del núcleo de contabilidad agregando columnas a `chart_of_accounts` (`level`, `nature`, `isTransactional`) para estructurar de forma jerárquica las cuentas principales y de registro. Se introdujeron las tablas `accounting_periods` para bloqueos de cierres fiscales periódicos y `accounting_mappings` para plantillas de cuentas puente automáticas.
- **Seeder y Mapeos Automáticos**: Al iniciar o consultar la contabilidad de una empresa por primera vez, el sistema autoejecuta un seeder del catálogo de cuentas estándar de la República Dominicana estructurado en árbol de múltiples niveles con naturalezas contables correctas (activo, pasivo, capital, ingresos y gastos) y precarga los mapeos puente para automatizar la facturación, ITBIS, cobros, inventario y cuentas por pagar sin requerir configuración manual inicial.
