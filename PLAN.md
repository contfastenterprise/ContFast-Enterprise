...
El proyecto se encuentra **Verified & Polished** tras completar la alineación exacta de clases CSS, rellenos (`px-7 py-3.5`), contenedores `relative flex`, íconos y transiciones del botón de guardado de cotizaciones con el de facturación (`invoices/page.tsx`), agregar el método estático de alias `PdfGenerator.generatePdf` para garantizar compatibilidad retroactiva total y resolver el caché de módulos de Turbopack/Next.js al generar impresiones en PDF, la homologación estética exacta del botón dividido de guardado y menú desplegable de cotizaciones con el diseño de facturación (utilizando la paleta de dorado `#C5A059`, bordes redondeados `rounded-xl`, menú contextual blanco con íconos retroalimentados e íconos en contenedores redondeados `bg-amber-100` y `bg-emerald-100`), la creación de las rutas API `/api/v1/quotes/[id]/print` y `/api/v1/quotes/[id]/pdf` que generan y transmiten la plantilla PDF de cotizaciones de forma nativa (`application/pdf`) mediante Puppeteer (homologado con el flujo de facturación), la creación de la plantilla A4 profesional para Cotizaciones (`DocumentTemplates.renderQuote`), la adición del botón de impresión de cotizaciones en la columna de acciones del listado (`/dashboard/quotes`), la implementación del botón desplegable multitarea ("Guardar Cotización" y "Guardar e Imprimir") en la creación y edición de cotizaciones, la implementación de la regla de control de ventas basada en stock mínimo en el registro de facturas, la incorporación de tipos de gastos dinámicos y base de datos multi-tenant con RLS (Row Level Security), la re-estructuración de la base de datos para sembrar clasificaciones de la DGII en la creación de inquilinos, la carga dinámica y resolución de nombres en el frontend y en las plantillas de impresión de facturas y reportes, el reinicio automático del formulario de compras al presionar la pestaña de registrar, la activación de políticas RLS en las tablas `expense_types` y `barcode_print_logs`, la creación de un panel de administración interactivo para tipos de gastos personalizados/estándares en Ajustes del Sistema, la validación estricta de formatos NCF (B, 11 chars) y e-NCF (E, 13 chars) en frontend/backend mediante expresiones regulares y helpers utilitarios, la optimización de consultas del catálogo de gastos de cada inquilino mediante almacenamiento en caché en Redis con invalidación transaccional, el desarrollo del módulo WMS de reordenes con límites de stock mínimo/máximo configurables por almacén en el catálogo, un panel visual premium de sugerencias de compra para reabastecimiento con KPIs, la integración directa con pre-carga de datos en el formulario de compras, y la incorporación del campo de "Método de Pago" con sus respectivas iniciales (E, C, T) y leyenda descriptiva en la tabla de la página de facturas y en el reporte de facturación en PDF.
...
- **Sincronización Dinámica de Entorno**: El ambiente de trabajo se deriva de forma estricta y dinámica desde el selector **Ambiente de Facturación (e-CF)** en la pestaña **Configuración Empresa** de `/dashboard/settings`. Al guardar los cambios, la aplicación autodetecta la discrepancia, sincroniza la cookie `cf_environment` y refresca la interfaz del panel automáticamente.
- **Propagación Segura de Cabeceras (Proxy)**: Corrección en el flujo de verificación del Access Token del proxy (`src/proxy.ts`). Anteriormente, no se inyectaba la cabecera `x-environment` en el request clonado bajo tokens válidos activos. Se configuró la lectura de la cookie `cf_environment` para inyectarla de forma consistente como `x-environment` en todos los flujos de llamada a APIs.
- **Aislamiento en Métricas y Dashboards**: Refactorización de `DashboardRepository`, `FinancialRepository` y `FinancialMovementService.autoSeedMovements` para aceptar e inyectar el parámetro `modo` en todas las consultas Drizzle y SQL crudas. Esto garantiza que tanto los indicadores de ventas, facturas pendientes y actividad reciente del inicio, como los balances, cuentas por cobrar/pagar e historial del dashboard financiero reflejen exclusivamente los datos correspondientes al modo Sandbox o Producción activo.
- **Interfaz de Usuario Visual Premium**: Integración de una franja de advertencia animada gruesa con rayas de peligro en color rojo/naranja cuando la empresa trabaja en `SANDBOX`, y una franja verde esmeralda discreta en `PRODUCCIÓN`. Se integró un badge premium de información en el navbar superior que enseña de manera clara e ilustrativa el ambiente actual.
...
### 65. Limpieza y Borrado de Datos de Prueba (Sandbox)
- **API de Purga Segura (Backend)**: Creación del endpoint de administración POST `/api/v1/admin/companies/[id]/clear-sandbox` que requiere autenticación y el rol de `sistemas`. Este endpoint ejecuta una transacción SQL ordenada que elimina de manera encadenada los registros transaccionales (facturas, cobros, pagos, contabilidad, nóminas, inventario, secuencias) donde `companyId = id` y `modo = 'PRUEBA'`, previniendo conflictos de claves foráneas y aislando totalmente los datos reales de Producción.
- **Acción Administrativa y Doble Confirmación (Frontend)**: Integración de un botón de limpieza (`RefreshCw` animable) en la tabla de `/dashboard/admin/companies`. Para evitar eliminaciones accidentales, se implementó una confirmación de texto interactiva (`prompt`) que obliga al administrador de sistemas a escribir exactamente el nombre de la empresa seleccionada antes de enviar el comando de red.
- **Aislamiento en API de Compras y Gastos**: Refactorización de los endpoints GET/POST/PUT/DELETE de `src/app/api/v1/expenses/route.ts` y `src/app/api/v1/expenses/[id]/route.ts`. Se inyectó el filtro `eq(expenses.modo, session.modo)` en las búsquedas y listados, y se aseguró la propagación de `modo: session.modo` al crear o editar gastos, existencias de inventario, movimientos de stock, cuentas por pagar y pólizas contables, eliminando por completo cualquier persistencia o fuga de datos entre Sandbox y Producción en el módulo de compras.
