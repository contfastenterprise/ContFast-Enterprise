...
El proyecto se encuentra **Verified & Polished** tras completar la unificación y homologación 1:1 de la plantilla de impresión de cotizaciones (`DocumentTemplates.renderQuote`) con el formato exacto de la factura (utilizando los mismos estilos CSS `@page`, contenedores `.invoice-wrapper`, encabezado `.company-info` con puntos para RNC/Teléfono/Email/Dirección, sección de cliente `.client-info` con `Razon Social`, `RNC/Cédula`, `Teléfono` y `Dirección` alineados a 18 caracteres, barra de condición `.condition-bar`, agrupación de líneas por almacén y categoría, tabla `.invoice-table` de 8 columnas, cuadro `.bottom-section` con notas y cuadro `.totals-table` con `TOTAL NETO` y firmas inferiores), la alineación exacta de clases CSS del botón de guardado, la adición del método de alias `PdfGenerator.generatePdf`, la creación de las rutas API `/api/v1/quotes/[id]/print` y `/api/v1/quotes/[id]/pdf`, el botón desplegable multitarea ("Guardar Cotización" y "Guardar e Imprimir"), la regla de control de ventas basada en stock mínimo, la incorporación de tipos de gastos dinámicos y RLS multi-tenant.
