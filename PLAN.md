...
### 37. OptimizaciÃ³n de LÃ­mite de Tasa (Rate Limiting) y Fallbacks
- **ProtecciÃ³n de Componentes y ReducciÃ³n de Peticiones**: Refactorizado `ClientLayout.tsx` y `layout.tsx` para cargar los ajustes de perfil y configuraciÃ³n de empresa directamente desde el lado del servidor, reduciendo el 50% de las peticiones iniciales del cliente al cargar la aplicaciÃ³n, evitando el error de "Too Many Requests" (HTTP 429).
- **LÃ³gica de Reintento con Backoff Exponencial**: Implementado `fetchWithRetry` en las llamadas crÃ­ticas de `page.tsx` del mÃ³dulo de facturas, capturando interceptivamente respuestas 429 para esperar y reintentar de forma invisible en caso de saturaciÃ³n momentÃ¡nea, sin quebrar la interfaz del usuario.
- **ExpansiÃ³n de LÃ­mite EstÃ¡ndar**: Se elevÃ³ el lÃ­mite de 100 a 500 peticiones por minuto en `rateLimiter.ts` para el preajuste `standard`, permitiendo un entorno de desarrollo mÃ¡s Ã¡gil con hot-reloading que naturalmente dispara mÃºltiples re-montajes debido al StrictMode de React.
