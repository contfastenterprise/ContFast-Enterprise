...
- **Direct Streaming & Consistencia de Formato**: Modificado `/api/v1/invoices/[id]/print` para soportar el método `GET`, el cual transmite directamente el archivo PDF original de la factura (`Content-Type: application/pdf` con `Content-Disposition: inline`) pregenerado al emitirse (utilizando el archivo físico de `pdfPath`). Esto garantiza consistencia de diseño al 100% y elimina los tiempos de renderizado de Puppeteer.
