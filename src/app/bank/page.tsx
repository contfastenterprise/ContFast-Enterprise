  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        {/* Bank List View */}
        <div className="bg-surface-container-low border border-outline-variant/30 rounded-lg shadow-lg overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-on-surface-variant">
              <thead className="bg-background/40 text-xs font-semibold text-on-surface-variant uppercase tracking-wider">
                <tr>
              ...
              <tbody className="divide-y divide-outline-variant/20">
                {loading ? (
                  <tr>
                    <td colSpan={6} className="py-10 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span className="text-on-surface-variant text-sm">Cargando cuentas bancarias...</span>
                      </div>
                    </td>
                  </tr>
                ) : accounts.length > 0 ? (
                  accounts.map((acc) => (
                    <tr key={acc.id} className="hover:bg-background/20 transition-colors">
                      <td className="py-4 px-6 font-semibold text-primary">{acc.bankName}</td>
                  ...
                  <tr>
                    <td colSpan={6} className="py-12 text-center text-on-surface-variant/70">
          ...
          {showModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <motion.div
                ...
                className="relative bg-surface-container-low border border-outline-variant/30 rounded-lg p-6 max-w-md w-full shadow-2xl z-10 text-on-surface-variant"
              >
                ...
                <form onSubmit={handleSave} className="space-y-4">
                  <div className="space-y-1">
                    <label className="block text-xs font-semibold text-on-surface-variant uppercase">Nombre del Banco</label>
                    <input
                      type="text"
                      required
                      value={bankName}
                  ...
                  <div className="space-y-1">
                    <label className="block text-xs font-semibold text-on-surface-variant uppercase">Número de Cuenta</label>
                    <input
                      type="text"
                      required
                      value={accountNumber}
                      onChange={(e) => setAccountNumber(e.target.value)}
                      className="block w-full rounded-md border-0 bg-background py-2.5 px-3 text-primary ring-1 ring-inset ring-outline-variant/30 focus:ring-2 focus:ring-amber-500 outline-none text-sm font-mono"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="block text-xs font-semibold text-on-surface-variant uppercase">Tipo de Cuenta</label>
                      <select
                        value={type}
                    ...
                    <div className="space-y-1">
                      <label className="block text-xs font-semibold text-on-surface-variant uppercase">Moneda</label>
                      <select
                        value={currency}
                  ...
                  {!editingAccount && (
                    <div className="space-y-1">
                      <label className="block text-xs font-semibold text-on-surface-variant uppercase">Balance Inicial</label>
                      <input
                        type="number"
                        step="any"
                        value={balance}
                  ...
                  <div className="flex justify-end gap-3 pt-4 border-t border-outline-variant/30">
                    <button
                      type="button"
                      onClick={() => setShowModal(false)}
                      className="rounded border border-outline-variant/30 bg-background px-4 py-2 text-xs font-semibold text-on-surface-variant hover:bg-surface-container-low"
                    >
                      Cancelar
                    </button>
                    <button
