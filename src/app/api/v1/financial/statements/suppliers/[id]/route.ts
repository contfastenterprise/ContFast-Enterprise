...
export async function GET(
  ...
  try {
    ...
    if (!supplierId) {
      return NextResponse.json({ success: false, error: { code: 'BAD_REQUEST', message: 'ID del suplidor es requerido' } }, { status: 400 });
    }
    ...
    if ((error as Error).message === 'Suplidor no encontrado') {
      return NextResponse.json(
