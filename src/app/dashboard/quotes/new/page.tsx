  ...
  return (
    <div className="space-y-8 animate-fade-in-up pb-12 w-full max-w-none">
      ...
      <motion.div
        ...
        exit={{ opacity: 0, x: -20 }}
        className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm space-y-6"
      >
        ...
        <form onSubmit={saveQuote} className="space-y-8">
          {/* General Settings */}
          <div className="bg-slate-50/40 p-4 rounded-xl border border-slate-200 space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div className="hidden md:grid md:grid-cols-[3fr_0.8fr_1.5fr_1.1fr_1fr_1.4fr_0.5fr] gap-3 px-3 py-2 bg-slate-100/80 text-[#003366] text-[10px] font-bold uppercase tracking-wider rounded-lg border border-slate-200">
            ...
            <div className="space-y-3">
              {lines.map((line, idx) => {
                ...
                return (
                  <div key={idx} className="grid grid-cols-1 md:grid-cols-[3fr_0.8fr_1.5fr_1.1fr_1fr_1.4fr_0.5fr] gap-3 items-center bg-slate-50/60 p-3 md:py-2 md:px-3 rounded-xl border border-slate-200">
                    ...
                    <div className="space-y-1.5 md:space-y-0">
                      ...
                      <input
                        ...
                        disabled={!hasProduct}
                        className={`w-full h-8 rounded-lg border px-3 py-1.5 outline-none text-xs font-mono-data text-right transition ${!hasProduct ? 'bg-slate-100 border-slate-300 text-[#003366]/50 cursor-not-allowed' : 'bg-white border-slate-300 text-[#003366] focus:border-[#C5A059]'}`}
                    ...
                    <div className="space-y-1.5 md:space-y-0">
                      ...
                      <input
                        ...
                        className={`w-full h-8 rounded-lg border px-3 py-1.5 outline-none text-xs font-mono-data text-right transition ${(!hasProduct || !canEditDiscount) ? 'bg-slate-100 border-slate-300 text-[#003366]/50 cursor-not-allowed' : 'bg-white border-slate-300 text-[#003366] focus:border-[#C5A059]'}`}
                        min={0} step="any"
                      />
                    </div>
                    ...
                    <div className="space-y-1.5 md:space-y-0">
                      ...
                      */}
                      <select
                        ...
                        disabled={!hasProduct}
                        className={`w-full h-8 rounded-lg border px-3 py-1.5 outline-none text-xs transition ${!hasProduct ? 'bg-slate-100 border-slate-300 text-[#003366]/50 cursor-not-allowed' : 'bg-white border-slate-300 text-[#003366] focus:border-[#C5A059]'}`}
                      >
                    ...
                    <div className="space-y-1.5 md:space-y-0 text-right">
                      ...
                      <span className="text-xs font-mono-data font-bold text-[#003366] pr-2">
          ...
          {/* Notes */}
          <div className="bg-slate-50/40 p-4 rounded-xl border border-slate-200 space-y-2">
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-3 py-1.5 text-xs rounded-lg border border-slate-200 bg-slate-50 font-medium focus:border-[#c5a059] focus:ring-1 focus:ring-[#c5a059]/20 outline-none transition resize-y placeholder:text-slate-500"
            />
          </div>

          {/* Calculation Summary & Submit */}
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-t border-slate-200 pt-4">
            <div className="bg-slate-50/60 p-4 rounded-xl border border-slate-200 w-full md:max-w-sm space-y-2 text-xs text-slate-700">
              <div className="flex justify-between">
                <span>Subtotal:</span>
                <span className="font-semibold font-mono-data text-[#003366]">RD$ {totals.subtotal.toLocaleString('es-DO', { minimumFractionDigits: 2 })}</span>
              </div>
              <div className="flex justify-between">
                <span>Descuento:</span>
                <span className="font-semibold font-mono-data text-[#003366]">RD$ {totals.discount.toLocaleString('es-DO', { minimumFractionDigits: 2 })}</span>
              </div>
              <div className="flex justify-between text-on-surface-variant/80">
                <span>Impuestos (ITBIS):</span>
                <span className="font-semibold font-mono-data text-[#003366]">RD$ {totals.tax.toLocaleString('es-DO', { minimumFractionDigits: 2 })}</span>
              </div>
              <div className="flex justify-between border-t border-slate-200 pt-3 mt-3 text-lg font-bold">
                ...
                <span className="text-emerald-500 font-mono-data">RD$ {totals.total.toLocaleString('es-DO', { minimumFractionDigits: 2 })}</span>
              </div>
            </div>
            ...
            <div className="flex flex-col-reverse sm:flex-row gap-3 w-full md:w-auto">
              ...
              <div className="relative flex items-center h-8 shadow-md rounded-lg">
                <button
                  ...
                  className="flex items-center justify-center gap-2 rounded-l-lg bg-[#C5A059] px-3 py-1.5 h-full text-xs font-bold text-slate-950 hover:bg-[#b08c4a] disabled:opacity-50 transition active:scale-[0.98]"
                >
                  {submitting ? (
