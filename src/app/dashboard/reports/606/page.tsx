  ...
  const cargarPresentacion = async () => {
    try {
      ...
      const marca = (data.data || []).find(
        (m: { tipo: string; periodo: string }) => m.tipo === '606' && m.periodo === periodoCompacto(period)
      );
  ...
  const alternarPresentada = async () => {
    ...
    try {
      ...
      const res = presentadaEn
        ? await fetch(`/api/v1/reports/declaraciones?tipo=606&periodo=${periodo}`, { method: 'DELETE' })
        : await fetch('/api/v1/reports/declaraciones', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ tipo: '606', periodo }),
          });
      const data = await res.json();
