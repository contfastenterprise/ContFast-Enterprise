...
export default function ReportsPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/v1/suppliers').then(res => res.json()).then(data => {
      if (data.success && data.data) {
  ...
  const handleGeneratePdf = async (type: string) => {
    ...
    try {
      ...
      if (type === 'ap_statement') {
        if (!selectedSupplierId) { toast.error('Debe seleccionar un proveedor'); setLoadingType(null); return; }
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-4 font-sans">
      ...
      <div className="space-y-8">
        ...
        {/* Grupo: Cuentas por Cobrar y Pagar */}
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition flex flex-col h-full relative z-10">
              ...
              <div className="flex flex-col flex-1 gap-2 px-4 pt-3 pb-4">
                <div className="flex items-center gap-2">
                  <BookOpen className="h-4 w-4 flex-shrink-0 text-rose-500" />
                  <p className="text-sm font-bold text-slate-800 leading-tight">Estado de Cuentas por Pagar</p>
                </div>
                <p className="text-[10px] text-slate-400 leading-snug">Facturas pendientes y balances adeudados a un proveedor.</p>
                <div className="flex-1">
        ...
        {/* Grupo: Impuestos e Institucional */}
        <div>
          <h2 className="text-lg font-bold text-slate-800 mb-3 flex items-center gap-2"><FileText className="w-5 h-5 text-blue-600"/> Impuestos (DGII)</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        {/* Grupo: Bancos */}
        <div>
          <h2 className="text-lg font-bold text-slate-800 mb-3 flex items-center gap-2"><Building className="w-5 h-5 text-indigo-600"/> Bancos y Conciliación</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
