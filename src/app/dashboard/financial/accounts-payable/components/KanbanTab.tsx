...
export default function KanbanTab({ data }: { data: any[] }) {
  const columns = useMemo(() => {
    const cols = {
      alDia: { title: 'Pendiente', icon: <CheckCircle className="w-4 h-4 text-emerald-500" />, items: [] as any[] },
      porVencer: { title: 'Próxima a vencer', icon: <Clock className="w-4 h-4 text-sky-500" />, items: [] as any[] },
      venceHoy: { title: 'Vence Hoy', icon: <Calendar className="w-4 h-4 text-amber-500" />, items: [] as any[] },
      vencidas: { title: 'Vencida', icon: <AlertCircle className="w-4 h-4 text-rose-500" />, items: [] as any[] },
    ...
    data.forEach(item => {
      ...
      if (item.status === 'scheduled') {
        cols.programada.items.push({ ...item, diffDays });
      } else if (diffDays > 0) {
