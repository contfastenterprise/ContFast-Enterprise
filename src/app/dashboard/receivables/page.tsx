            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl relative z-10 flex flex-col max-h-[90vh] overflow-hidden">
                <div className="bg-[#003366] px-6 py-5 flex justify-between items-center shrink-0">
                  <div>
                    <h3 className="text-primary font-bold text-lg flex items-center gap-2"><HandCoins className="w-5 h-5 text-[#C5A059]" /> Registrar Recibo de Cobro</h3>
                    <p className="text-blue-200 text-sm mt-0.5">{selectedCustomer.customerName}</p>
                  </div>
                  <button onClick={() => setShowPaymentModal(false)} className="text-primary/70 hover:text-primary"><X className="w-6 h-6" /></button>
                </div>
                ...
                <div className="flex flex-col md:flex-row overflow-hidden flex-1">
                  ...
                  <div className="md:w-1/3 bg-slate-50 border-r border-slate-200 p-6 space-y-5 overflow-y-auto shrink-0">
                    <div>
                      <label className="block text-xs font-bold text-on-surface-variant/70 uppercase tracking-widest mb-1.5"><Calendar className="w-3 h-3 inline mr-1"/> Fecha de Cobro</label>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-on-surface-variant/70 uppercase tracking-widest mb-1.5"><CreditCard className="w-3 h-3 inline mr-1"/> Método de Pago</label>
                      <div className="grid grid-cols-2 gap-2">
                        <button type="button" onClick={() => setPaymentForm({...paymentForm, paymentMethod: 'bank'})} className={clsx("py-2 px-3 rounded-lg border text-sm font-semibold flex items-center justify-center gap-2 transition-all", paymentForm.paymentMethod === 'bank' ? 'bg-[#003366] border-[#003366] text-primary' : 'bg-white border-slate-200 text-on-surface-variant/80 hover:bg-slate-50')}>
                           <Landmark className="w-4 h-4" /> Banco
                        </button>
                        <button type="button" onClick={() => setPaymentForm({...paymentForm, paymentMethod: 'cash'})} className={clsx("py-2 px-3 rounded-lg border text-sm font-semibold flex items-center justify-center gap-2 transition-all", paymentForm.paymentMethod === 'cash' ? 'bg-[#003366] border-[#003366] text-primary' : 'bg-white border-slate-200 text-on-surface-variant/80 hover:bg-slate-50')}>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-on-surface-variant/70 uppercase tracking-widest mb-1.5">Monto Recibido</label>
                      ...
                      <p className="text-[10px] text-on-surface-variant mt-1">El monto se auto-distribuye en las facturas más antiguas.</p>
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-on-surface-variant/70 uppercase tracking-widest mb-1.5">Referencia (Cheque/Transfer)</label>
                  ...
                  <div className="md:w-2/3 bg-white flex flex-col overflow-hidden">
                    <div className="px-6 py-4 border-b border-slate-100 bg-slate-50 shrink-0">
                      ...
                      <p className="text-xs text-on-surface-variant/70">Distribuye el monto recibido en las facturas pendientes a continuación.</p>
                    </div>
                    <div className="flex-1 overflow-y-auto p-4">
                      <table className="w-full text-sm text-left">
                        <thead className="text-[10px] text-on-surface-variant uppercase font-bold tracking-widest border-b border-slate-100">
                          <tr>
                          ...
                          {selectedCustomer.invoices.map(inv => {
                            ...
                            return (
                              <tr key={inv.arId} className={clsx("transition-colors", applied > 0 ? 'bg-emerald-50/50' : '')}>
                                ...
                                <td className="py-3 text-right text-on-surface-variant/70">{new Date(inv.dueDate).toLocaleDateString('es-DO')}</td>
                    ...
                    <div className="bg-slate-50 border-t border-slate-200 p-6 shrink-0 flex items-center justify-between">
                      ...
                      <button type="button" onClick={handleSubmitPayment} disabled={submitting} className="bg-[#C5A059] hover:bg-[#b08c4a] text-primary font-bold py-3 px-8 rounded-xl shadow-md transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
