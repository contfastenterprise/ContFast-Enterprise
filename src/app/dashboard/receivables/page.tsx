...
export default function ReceivablesPage() {
  ...
  // Autocomplete state for customer selector
  const [customerSearchInput, setCustomerSearchInput] = useState('');
  const [showCustomerDropdown, setShowCustomerDropdown] = useState(false);
  const customerDropdownRef = useRef<HTMLDivElement>(null);

  const filteredStatementCustomers = useMemo(() => {
    if (!customerSearchInput.trim()) return statementCustomers;
    const q = customerSearchInput.toLowerCase();
    return statementCustomers.filter(c =>
      c.name?.toLowerCase().includes(q) ||
      c.rncCedula?.toLowerCase().includes(q)
    );
  }, [statementCustomers, customerSearchInput]);
    ...
    <div className="min-h-full bg-slate-50 text-slate-900 font-sans pb-20 max-w-7xl mx-auto w-full">
      ...
      <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
        ...
        {activeTab === 'pending' && (
          <>
            {/* Search Bar */}
            <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-200 flex flex-col sm:flex-row gap-3 items-center">
        ...
        {activeTab === 'receipts' && (
          <>
            ...
            <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-200 flex flex-col md:flex-row gap-3 items-end">
              <div className="relative flex-1 w-full">
                <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase tracking-wider">Buscar por Cliente o Referencia</label>
              ...
              <div className="w-full md:w-40">
                <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase tracking-wider">Desde</label>
                <input
                  type="date"
              ...
              <div className="w-full md:w-40">
                <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase tracking-wider">Hasta</label>
                <input
                  type="date"
              ...
              {receiptsLoading ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left">
                    <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
                        <th className="px-4 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Código Recibo</th>
                        ...
                        <th className="px-4 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Método</th>
                        ...
                        <th className="px-4 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-center">Acciones</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {filteredReceipts.map((rec) => {
                        ...
                        return (
                          <tr key={rec.id} className="hover:bg-slate-50/60 transition-colors">
                            <td className="px-4 py-2.5 font-mono font-bold text-xs text-[#003366]">
                            ...
                            <td className="px-4 py-2.5 font-mono text-xs text-slate-500">{rec.reference || '-'}</td>
        ...
        {activeTab === 'customer_statement' && (
          <>
            ...
            <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-200 flex flex-col md:flex-row gap-3 items-end">
              <div className="flex-1 w-full" ref={customerDropdownRef}>
                <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase tracking-wider">Seleccionar Cliente</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  ...
                  {customerSearchInput && (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setSelectedStatementCustomerId('');
                      ...
                      {filteredStatementCustomers.length === 0 ? (
                        ...
                        filteredStatementCustomers.map(c => (
                          <button
                            ...
                            type="button"
                            onClick={() => {
                              setSelectedStatementCustomerId(c.id);
                              setCustomerSearchInput(`${c.name}${c.rncCedula ? ` (${c.rncCedula})` : ''}`);
                              setShowCustomerDropdown(false);
                              fetchCustomerStatement(c.id);
                            }}
                            className={clsx(
                              "w-full text-left px-3 py-2 text-xs hover:bg-slate-50 transition-colors flex items-center gap-2 border-b border-slate-100 last:border-0",
                              selectedStatementCustomerId === c.id && "bg-[#003366]/5 font-bold"
                            )}
                          >
                            <Building2 className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                            <div className="min-w-0">
                              <span className="block font-semibold text-slate-700 truncate">{c.name}</span>
                              {c.rncCedula && <span className="block text-[10px] text-slate-400 font-mono">{c.rncCedula}</span>}
                            </div>
                          </button>
                        ))
                      )}
              ...
              <div className="w-full md:w-64">
                <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase tracking-wider">Filtro de búsqueda (Factura, Recibo, Ref)</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <Search className="h-3.5 w-3.5 text-slate-400" />
                  </div>
                ...
                <div className="overflow-x-auto print:overflow-visible">
                  ...
                  <table className="w-full text-xs text-left">
                    <thead className="bg-slate-50 border-b border-slate-200 print:bg-transparent">
                      <tr>
                        <th className="px-4 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Fecha</th>
                        <th className="px-4 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Recibo</th>
                        <th className="px-4 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Factura / NCF</th>
                        <th className="px-4 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Mapeo de Pago</th>
                        <th className="px-4 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-right">Monto Factura</th>
                        <th className="px-4 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-right">Monto Aplicado (Abono)</th>
                        <th className="px-4 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-right text-rose-600">Balance Restante</th>
                        <th className="px-4 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-center print:hidden">Acciones</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {(() => {
                        ...
                        return filteredFinal.map((item) => {
                          ...
                          return (
                            <tr key={item.appliedId} className="hover:bg-slate-50/60 transition-colors">
                              ...
                              <td className="px-4 py-2.5">
                                <div className="font-semibold text-slate-700">{item.codigoFactura || 'N/A'}</div>
              ...
              <div className="flex flex-col md:flex-row overflow-hidden flex-1">
                ...
                <div className="md:w-1/4 bg-slate-50/50 border-r border-slate-200 p-6 space-y-5 overflow-y-auto shrink-0">
                  <div>
                    ...
                    <input type="date" required value={paymentForm.date} onChange={e => setPaymentForm({ ...paymentForm, date: e.target.value })} className="w-full bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-xs font-semibold focus:ring-1 focus:ring-[#c5a059]/20 focus:border-[#c5a059] outline-none text-slate-800 transition-colors" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-slate-200 rounded-lg px-3 py-1.5 outline-none focus:ring-1 focus:ring-[#c5a059]/20 focus:border-[#c5a059] text-xs font-semibold text-slate-800 transition-colors disabled:bg-slate-100 disabled:cursor-not-allowed disabled:text-slate-400"
                  ...
                  <div>
                    ...
                    <textarea value={paymentForm.notes} onChange={e => setPaymentForm({ ...paymentForm, notes: e.target.value })} rows={2} className="w-full bg-white border border-slate-200 rounded-lg px-3 py-1.5 outline-none focus:ring-1 focus:ring-[#c5a059]/20 focus:border-[#c5a059] text-xs font-semibold text-slate-800 resize-none transition-colors"></textarea>
                  </div>
                </div>
