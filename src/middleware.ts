...
async function verifyHS256(token: string, secret: string): Promise<any | null> {
  try {
    ...
    console.error('[Middleware-Auth] Error verifying token:', err);
    return null;
  }
}
...
export async function middleware(req: NextRequest) {
  ...
  // 2. Route Protection Target: /dashboard/* and /api/v1/*
  ...
  if (isDashboardRoute || isApiRoute) {
    ...
    if (refreshToken) {
      console.log(`[Middleware] Access token expired or missing. Attempting token rotation for path: ${pathname}`);
      try {
        ...
        if (refreshResponse.ok) {
          ...
          if (refreshResult.success) {
            console.log('[Middleware] Session rotated successfully.');
            ...
            // Construct response (either rewrite to path or redirect)
        ...
        console.error('[Middleware-Auth] Error during token rotation fetch:', err);
      }
    }
    ...
    if (isApiRoute) {
      ...
      // For dashboard pages, clear active cookies and redirect to login page
